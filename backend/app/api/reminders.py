...
# Gestion des rappels medicaux avec notifications
...
from app.api.consultations import create_notification

# redirect_slashes=False empêche les redirections automatiques de FastAPI
# qui peuvent causer des problèmes CORS/méthode (405 Method Not Allowed)
router = APIRouter(prefix="/patient", tags=["patient"], redirect_slashes=False)
...
def init_reminders_tables():
    """Cree les tables pour les rappels medicaux"""
    conn = get_db()
    cur = conn.cursor()
    cur.execute("""
    ...
    cur.close()
    conn.close()
    print("✅ [Reminders] Table reminders creee avec succes")
...
# ============================================================================
# ROUTES SANS TRAILING SLASH (compatibilite frontend)
# redirect_slashes=False dans APIRouter empêche FastAPI de rediriger
# ============================================================================
...
async def create_reminder(
    ...
    """Cree un nouveau rappel medical et envoie une notification"""
    if current_user["role"] not in ("Patient", "Administrateur"):
        raise HTTPException(403, "Acces reserve aux patients.")
    ...
    time_str = f" a {reminder.time}" if reminder.time else ""
    create_notification(
        current_user["id"],
        "new_reminder",
        "📅 Nouveau rappel medical",
        f"'{reminder.title}' prevu le {reminder.date}{time_str}",
        {"reminder_id": reminder_id, "type": reminder.type}
    )
...
async def update_reminder(
    ...
    """Met a jour un rappel (toggle completed, etc.)"""
    if current_user["role"] not in ("Patient", "Administrateur"):
        raise HTTPException(403, "Acces reserve aux patients.")
    ...
    if update.completed is True:
        create_notification(
            ...
            "reminder_completed",
            "✅ Rappel termine",
            f"Vous avez marque '{existing['title']}' comme termine.",
...
async def delete_reminder(
    ...
    create_notification(
        ...
        "reminder_deleted",
        "🗑️ Rappel supprime",
        f"Le rappel '{existing['title']}' a ete supprime.",
        {"reminder_id": reminder_id}
    )
...
async def check_upcoming_reminders(
    ...
    """Verifie les rappels a venir et envoie des notifications"""
    if current_user["role"] not in ("Patient", "Administrateur"):
        raise HTTPException(403, "Acces reserve aux patients.")
    ...
    for reminder in upcoming:
        ...
        time_str = f" a {reminder['reminder_time']}" if reminder.get("reminder_time") else ""
        create_notification(
            current_user["id"],
            "reminder_upcoming",
            "⏰ Rappel a venir",
            f"'{reminder['title']}' est prevu pour le {reminder['reminder_date']}{time_str}",
            {"reminder_id": reminder["id"], "type": reminder.get("type", "medical")}
        )
        notified_count += 1
